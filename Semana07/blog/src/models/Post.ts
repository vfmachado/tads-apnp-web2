...
export class Post  {
    ...
    static save(post: IPost, userID: number, callback: Function): void {
        
        if (!post.publishedDate) {
            post.publishedDate = new Date();
        }

        let sql = `INSERT INTO post (title, text, publishedDate, user_id) VALUES (?, ?, ?, ?);`
        db.run(sql, [post.title, post.text, post.publishedDate, userID], (err) => {
            if (err) {
